"""
Задача:
Целое положительное число называется простым, если оно имеет ровно два различных
делителя, то есть делится только на единицу и на само себя. Например, число 2 является
простым, так как делится только на 1 и 2.
...
def primes():
    ...
    i = 3
    while True:
        if not any(list(map(lambda j: i % j == 0, lst))):
            lst.append(i)
            yield lst[-1]
...
# Проверка
y = primes()
print (next(y)) # Выводит 2
print (next(y)) # Теперь выводит 3
print (next(y)) # Теперь выводит 5